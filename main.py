...
def print_advisor_list():
    ...
    print(f"\n{Colors.BOLD}📋 Available Advisors:{Colors.END}\n")
...
def print_board_results(results: dict, synthesis: Optional[str] = None):
    ...
    print(f"{Colors.BOLD}📊 BOARD OF ADVISORS RESPONSES{Colors.END}")
    ...
    if synthesis:
        ...
        print(f"🎯 {Colors.BOLD}SYNTHESIZED ACTION PLAN{Colors.END}")
...
def interactive_mode(client: Anthropic):
    ...
    print(f"\n{Colors.BOLD}🏛️  BUSINESS BOARD OF ADVISORS  🏛️{Colors.END}")
    ...
    while True:
        ...
        if choice == "1":
            ...
            print(f"\n{Colors.CYAN}Thanks for consulting the board! 👋{Colors.END}\n")
            break
        else:
